...
class Board:
    ...
    def move_and_promote_pawn(self, row, col, row1, col1, char):
        if isinstance(self.get_piece(row, col), Pawn):
            if abs(row1 - row) == 1 and row1 == 0 or row1 == 7:
                if self.move_piece(row, col, row1, col1):
                    color = self.field[row1][col1].get_color()
                    if char == 'R':
...
class Knight:
    ...
    def can_move(self, board, row, col, row1, col1):
        ...
        if row == row1 and col == col1:
            return False
        if not (abs(row - row1) == 1 and abs(col - col1) == 2 or abs(row - row1) == 2 and abs(
                col - col1) == 1):
            return False
...
class King:
    ...
    def can_move(self, board, row, col, row1, col1):
        if not correct_coords(row, col):
            return False
        if row != row1 and col != col1 and not (abs(row - row1) == abs(col - col1)):
            return False
        if not abs(row - row1) <= 1 or not abs(col - col1) <= 1:
            return False
        return True
    ...
    def can_move(self, board, row, col, row1, col1):
        ...
        if row + col == row1 + col1:
            ...
            for r in range(row + step_y, row1, step_y):
                ...
                if not (board.get_piece(r, col + row - r) is None):
                    return False
            return True
        if row - col == row1 - col1:
            step_y = 1 if (row1 >= row) else -1
            for r in range(row + step_y, row1, step_y):
                # Если на пути по диагонали есть фигура
                if not (board.get_piece(r, col + r - row) is None):
                    return False
            return True
        return False
